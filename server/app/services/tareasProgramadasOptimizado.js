    ...
    try {
      ...
      for (const usuario of usuarios) {
        try {
          ...
          // Obtener avances del usuario sin orderBy para evitar índices complejos
          ...
          if (!avancesSnapshot.empty) {
            ...
            avances.sort((a, b) => new Date(b.fechaHora) - new Date(a.fechaHora));
            if (avances.length > 0) {
              ultimoAvance = new Date(avances[0].fechaHora);
            }
          }
      ...
      for (const usuario of usuarios) {
        try {
          ...
          if (avancesSnapshot.empty) {
            ...
            // Ordenar por fecha manualmente
            avances.sort((a, b) => new Date(b.fechaHora) - new Date(a.fechaHora));
            const ultimoAvance = new Date(avances[0].fechaHora);
